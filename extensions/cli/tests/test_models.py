...
class TestCategoriesConstants:
    ...
    def test_valid_ccf_ranks(self):
        assert "A" in VALID_CCF_RANKS
        assert "B" in VALID_CCF_RANKS
        assert "X" not in VALID_CCF_RANKS
...
class TestAccYear:
    def test_from_dict(self):
        data = {"year": 2024, "submitted": 1000, "accepted": 250, "str": "25.0%", "rate": "25.0"}
        acc = AccYear.from_dict(data)
        assert acc.year == 2024
        assert acc.submitted == 1000
        assert acc.accepted == 250
        assert acc.info == "25.0%"


class TestConfAccRate:
    def test_from_dict(self):
        data = {
            "title": "CVPR",
            "accept_rates": [
                {"year": 2024, "submitted": 1000, "accepted": 250, "str": "25.0%", "rate": "25.0"}
            ],
        }
        acc = ConfAccRate.from_dict(data)
        assert acc.title == "CVPR"
        assert len(acc.accept_rates) == 1
